...
require './lib/fluiddb2/sqlite'

# PgsqlSQLTest
class SqlLiteTest < Minitest::Test
  def setup
    cs = 'sqlite:///tmp/fluiddb.test'
    @db = FluidDb2::SQLite.new(URI.parse(cs))
  ...
  def test_query_for_array
    ...
    resultset = @db.query_for_array(sql_in)
    h = Hash['f1', 1, 'f2', 'Two', 0, 1, 1, 'Two']
    assert_equal h, resultset
  end
  ...
  def test_query_for_resultset
    ...
    r = [Hash['f1', 1, 'f2', 'Two', 0, 1, 1, 'Two'],
         Hash['f1', 2, 'f2', 'Three', 0, 2, 1, 'Three']]
    assert_equal r, resultset
  end
